# Memory Archive — cca-exercises

_Overflow for `docs/memory.md`'s "Last Session Summary" section. `memory.md` keeps the most recent 3-5 entries; when it grows past that, older entries move here verbatim. Newest-first, like the source section. This file is not read by routine `/sync` runs. Most of what's below is also captured in the "Practice Exam Results" table in `memory.md`'s "Prior State" section — this archive preserves the narrative/miss-pattern detail the table doesn't carry._

_First archived: 2026-07-30 (moved all entries from 2026-05-07 through 2026-06-03)._
